...
function parseICSDate(value: string, keyWithParams = ''): Date {
  ...
  const raw = isUtc ? clean.slice(0, -1) : clean;
  if (raw.length <= 8) return new Date(y, mo, d);
  const h = Number(raw.slice(9, 11)) || 0;
  const mi = Number(raw.slice(11, 13)) || 0;
  const s = Number(raw.slice(13, 15)) || 0;
...
function parseRRule(value: string): RRule | null {
  ...
  const until = untilRaw ? parseICSDate(untilRaw) : undefined;
...
function expandRecurringEvent(base: {
  title: string;
  start: Date;
  end: Date;
  location?: string;
  ...
  let current = new Date(base.start);
  let occurrence = 0;
  const hardStop = new Date(base.start);
  hardStop.setFullYear(hardStop.getFullYear() + 10);
  ...
  while (occurrence < 1000 && current <= hardStop) {
    ...
    results.push({
      ...
      recurrenceType: 'none',
      externalUid: `${base.uid || base.title}|${current.toISOString()}`,
    });
    occurrence++;
    ...
    if (line === 'END:VEVENT') {
      if (current && current.start) {
        const base = {
          ...
          end: (current.end || current.start) as Date,
          location: current.location as string | undefined,
          description: current.description as string | undefined,
          uid: current.externalUid as string | undefined,
        };
        const rule = current.rrule ? parseRRule(current.rrule) : null;
        if (rule) results.push(...expandRecurringEvent({ ...base, rrule: rule }));
        else results.push({ ...base, recurrenceType: 'none', externalUid: base.uid || `${base.title}|${base.start.toISOString()}` });
      }
      current = null;
      continue;
    }
