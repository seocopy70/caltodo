...
export default function ImportExportPanel({ events, todos, notes, user, onNotify, onRefresh, onClose }: any) {
  ...
  return <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-50 p-4"><div className="bg-slate-900 border border-slate-700 w-full max-w-md rounded-3xl shadow-2xl overflow-hidden max-h-[90vh] overflow-y-auto"><div className="p-6 space-y-5"><div className="flex justify-between items-center"><h3 className="font-bold text-xl text-white">가져오기 / 내보내기</h3><button onClick={onClose} className="p-1 hover:bg-slate-800 rounded-full"><X /></button></div>
    {pendingJsonImport ? (
      <div className="space-y-4">
        <p className="text-sm">백업 파일에서 일정 <span className="font-bold text-blue-400">{pendingJsonImport.events.length}개</span>, 할 일 <span className="font-bold text-emerald-400">{pendingJsonImport.todos.length}개</span>, 메모 <span className="font-bold text-amber-400">{pendingJsonImport.notes.length}개</span>를 찾았어요.</p>
        <p className="text-xs text-slate-500">같은 항목을 다시 가져와도 원래 id로 덮어써져서 중복 생성되지 않아요.</p>
        <div className="flex gap-3"><button onClick={() => setPendingJsonImport(null)} className="flex-1 py-3 font-bold text-slate-400">취소</button><button disabled={importing} onClick={confirmJsonImport} className="flex-[2] py-3 bg-blue-600 text-white rounded-2xl font-bold">{importing ? '복원하는 중...' : '전체 복원하기'}</button></div>
      </div>
    ) : !pendingImport ? <>
      <div className="space-y-2">
        <p className="text-xs text-slate-500">구글 캘린더 · 삼성 캘린더에서 .ics 파일을 가져올 수 있어요. 같은 외부 일정은 다시 가져와도 중복 생성하지 않습니다.</p>
        <button onClick={() => fileInputRef.current?.click()} className="w-full flex items-center gap-3 bg-slate-800 p-4 rounded-2xl"><Upload className="w-5 h-5 text-blue-400" /><div className="text-left"><p className="font-bold text-sm">.ics 파일 가져오기</p><p className="text-[11px] text-slate-500">구글/삼성 캘린더에서 내보낸 파일 선택</p></div></button>
        <input ref={fileInputRef} type="file" accept=".ics,text/calendar" className="hidden" onChange={handleFileSelect} />
        <button onClick={() => jsonInputRef.current?.click()} className="w-full flex items-center gap-3 bg-slate-800 p-4 rounded-2xl"><DatabaseBackup className="w-5 h-5 text-violet-400" /><div className="text-left"><p className="font-bold text-sm">전체 백업 가져오기 (.json)</p><p className="text-[11px] text-slate-500">기기를 바꿨을 때 백업 파일로 그대로 복원</p></div></button>
