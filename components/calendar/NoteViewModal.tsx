    ...
    <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 w-full max-w-2xl rounded-3xl shadow-2xl overflow-hidden max-h-[85vh] flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 dark:border-slate-800 shrink-0">
          <h3 className="font-bold text-xl text-slate-900 dark:text-white truncate pr-3">{note.title}</h3>
        ...
        <div
          ...
          className={`p-6 overflow-y-auto flex-1 whitespace-pre-wrap text-lg leading-relaxed text-slate-800 dark:text-slate-100 ${editable ? 'cursor-text hover:bg-slate-50 dark:hover:bg-slate-800/20' : ''}`}
        >
          {note.content || <span className="text-slate-400 dark:text-slate-600">내용이 없습니다.</span>}
        </div>
