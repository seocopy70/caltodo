...
export default function HomeView({ events, todos, notes = [], user, onNotify, onRefresh, onPatchTodo, onRemoveTodo, onNewNote, onEditNote }: any) {
  ...
  return <div className="max-w-5xl mx-auto space-y-5 p-2">
    <TodoListPanel todos={todos} user={user} onNotify={onNotify} onRefresh={onRefresh} onPatchTodo={onPatchTodo} onRemoveTodo={onRemoveTodo} maxVisible={5} compact />
    ...
    <div className="flex justify-end gap-2">
      ...
      <button type="button" onClick={() => onNewNote?.()} className="flex items-center gap-2 px-3.5 py-2.5 rounded-xl border border-amber-500/30 bg-amber-500/10 text-amber-400 hover:bg-amber-500/20 transition font-bold text-sm" title="새 메모"><NotebookPen className="w-5 h-5" /><span className="hidden sm:inline">새 메모</span></button>
    </div>
    ...
    {notes.length > 0 && <section className="rounded-2xl border border-amber-500/20 bg-amber-500/5 overflow-hidden divide-y divide-amber-500/10">
      {notes.map((note: any) => (
        <button key={note.id} onClick={() => setViewingNote(note)} className="w-full text-left px-5 py-4 hover:bg-amber-500/10 transition">
          <div className="flex items-center gap-1.5 mb-1"><StickyNote className="w-4 h-4 text-amber-400 shrink-0" /><span className="font-bold text-base truncate">{note.title}</span></div>
          <p className="text-base text-slate-400 line-clamp-2 leading-relaxed">{note.content}</p>
        </button>
      ))}
    </section>}
    ...
    {viewingNote && <NoteViewModal note={viewingNote} onClose={() => setViewingNote(null)} onEdit={(n: any) => { setViewingNote(null); onEditNote?.(n); }} />}
  </div>;
}
