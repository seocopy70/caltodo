...
export default function NotesView({ notes, folders = [], user, onNotify, onRefresh, onNewNote, onEditNote, onPatchNote, onSwipeHint }: any) {
  ...
  return <div className="max-w-2xl mx-auto space-y-4 p-2" onTouchStart={handleFolderSwipeStart} onTouchEnd={handleFolderSwipeEnd}>
    ...
    <div className="flex items-center gap-2">
      ...
      <div className="relative shrink-0">
        ...
        {folderPickerOpen && (
          <>
            ...
            {/* onClick으로 닫음(pointerdown+preventDefault 방식은 되돌림, TodoView.tsx와 동일한 이유) */}
      ...
      <button onClick={() => setLayoutMode((m) => (m === 'card' ? 'title' : 'card'))} title={layoutMode === 'card' ? '제목 목록으로 보기' : '카드형으로 보기'} className="px-2 py-2.5 rounded-xl bg-slate-100 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/50 shrink-0 text-slate-500">
        {layoutMode === 'card' ? <List className="w-5 h-5" /> : <LayoutGrid className="w-5 h-5" />}
      </button>
    </div>

    {/* 실제로 만들어져 있는 폴더만 이름이 있는 색깔버튼으로 보여줘서 탭 한 번으로 그 폴더만 보기(할일탭과 동일한 형식) */}
    {folders.length > 0 && (
                ...
                <div className="flex items-center justify-between gap-2 mt-2">
                  ...
                  {folders.length > 0 && (() => { const fc = note.folderId ? getFolderColor(note.folderId, folders) : null; return (
                    <button
                      ...
                      title="폴더 선택"
                    >
                      <Folder className={`w-3.5 h-3.5 shrink-0 ${fc ? fc.text : 'text-slate-400 dark:text-slate-600'}`} />
                      <span className={`text-xs font-bold truncate ${fc ? fc.text : 'text-slate-400 dark:text-slate-600'}`}>{folders.find((f: any) => f.id === note.folderId)?.name || '미분류'}</span>
                    </button>
                  ); })()}
                </div>
              </div>
    ...
    {noteFolderPickerFor && noteFolderPickerAnchor && (
      <>
        ...
        {/* onClick으로 닫음(pointerdown+preventDefault 방식은 되돌림, 위와 동일한 이유) */}
