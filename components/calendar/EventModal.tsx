...
export default function EventModal({ date, editingEvent, user, notify, onClose }: any) {
  ...
  // 저장/삭제는 결과를 기다리지 않고 즉시 모달을 닫는다(낙관적 UI).
  // 네트워크 지연이나 Firestore 응답 지연에도 화면이 묶이지 않도록 함.
  // 실패했을 때만 토스트로 알려준다.
  const save = () => {
      ...
      <div className="bg-slate-900 border border-slate-700 w-full max-w-md rounded-3xl shadow-2xl overflow-hidden max-h-[90vh] overflow-y-auto">
        <div className="p-6 space-y-5">
          ...
          <div className="flex items-center gap-3 bg-slate-800 p-3 rounded-2xl">
            ...
            <select
              className="bg-slate-900 text-sm rounded-lg px-2 py-1.5 outline-none border border-slate-700"
