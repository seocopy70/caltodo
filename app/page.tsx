...
export default function Home() {
  ...
  return <div className="min-h-screen bg-slate-50 dark:bg-[#0f172a] text-slate-900 dark:text-slate-100">
    ...
    <main className="max-w-7xl mx-auto p-3 sm:p-5">{search.trim() ? <GlobalSearch query={search} events={events} todos={todos} notes={activeNotes} onClose={() => setSearch('')} onEvent={() => { setSearch(''); setView('list'); }} onTodo={(t: any) => { setSearch(''); setEditingTodo(t); }} onNote={() => { setSearch(''); setView('notes'); }} /> : view === 'today' ? <HomeView events={events} todos={todos} notes={todayNotes} user={user} onNotify={notify} onRefresh={refreshData} /> : view === 'month' ? <Calendar key="month-view" view="month" events={events} user={user} onRefresh={refreshData} onNotify={notify} /> : view === 'week' ? <Calendar key="week-view" view="week" events={events} user={user} onRefresh={refreshData} onNotify={notify} /> : view === 'list' ? <EventListView events={events} user={user} onRefresh={refreshData} onNotify={notify} /> : view === 'todo' ? <TodoView todos={todos} user={user} onNotify={notify} onRefresh={refreshData} /> : <NotesView notes={notes} user={user} onNotify={notify} onRefresh={refreshData} />}</main>
