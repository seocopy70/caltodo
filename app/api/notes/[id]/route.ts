...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  await turso.execute({
    sql: 'UPDATE notes SET title = ?, content = ?, updated_at = ?, show_today = ? WHERE id = ? AND deleted_at IS NULL',
    args: [body.title || '(제목 없음)', body.content || '', Date.now(), body.showToday ? 1 : 0, params.id],
  });
